...
The 'dbt-training' project was generated by using the dbt template for
...
See [SETUP.md](SETUP.md) for a step-by-step installation guide, [COURSE.md](COURSE.md) for the training outline and project layout, and [training/](training/README.md) for the student notebooks.
...
3. Install dbt
   ...
   Run these instructions from the `dbt-training` directory. We recommend making
...
`[dev yourname] dbt-training_job` to your workspace.
...
When deploying to this target, note that the default job at resources/dbt-training.job.yml
